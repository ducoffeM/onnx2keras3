...
from onnx2keras3.typing import Tensor, Node, Graph, WeightsOnnx, \
                                DataFormat, Attributes, Attribute, ArrayLike
...
def get_parent_by_name(node_input:str, onnx_nodes:List[Node], onnx_input:List[Node], onnx_input_names:List[str])->List[Node]:
...
def get_parents_by_name(node_inputs:List[str], onnx_nodes:List[Node], onnx_input:List[Node], onnx_input_names:List[str])->List[Node]:
...
def get_layer_name(node:Node)->str:
    output_name = node.output[0].split('/')[-1]
    return output_name
