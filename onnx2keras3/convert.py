...
from onnx2keras3.utils import onnx_node_attributes_to_dict, get_id, get_children_by_name, get_parents_by_name, get_layer_name
import keras
...
def iter_node(current_nodes:List[Node], dict_input_tensor:Dict[str, List[Tensor]], \
                onnx_nodes:List[Node], constant_nodes:List[Node], onnx_input:List[Node],\
                onnx_input_names:List[str], onnx_weights:List[WeightsOnnx])->List[Node]:
    """
    ...
    for k, current_node in enumerate(current_nodes):
        ...
        if get_id(current_node) in previous_nodes:
            ...
            raise ValueError('this {}-th node {} has been treated before, are we looping ?'.format(k, get_id(current_node)))
            ...
            parents_nodes:List[Node] = get_parents_by_name(current_node.input, onnx_nodes+constant_nodes, onnx_input, onnx_input_names)
            ...
            if check_input:
                ...
                for parent_node in parents_nodes:
                    ...
                    if len(p_output_tensors)>1:
                        ...
                        indices:List[int] = list(np.where(output_name in input_names for output_name in output_names)[0])
                ...
                # retrieve weights
                weights:List[ArrayLike] = [numpy_helper.to_array(onnx_weight) for onnx_weight in onnx_weights if onnx_weight.name in current_node.input]
                ...
                for child in children:
                    if get_id(child) in previous_nodes:
                        raise ValueError('this child {} has been treated before, are we looping ?'.format(get_id(current_node)))
...
def onnx_to_keras(filename:str)->KerasModel:
    ...
    # init current_nodes
    is_next_node:List[bool] = [max([input_name in onnx_node.input for input_name in onnx_input_names]) for onnx_node in onnx_nodes]
    current_nodes:List[Node] = [ onnx_nodes[i] for i in np.where(is_next_node)[0]]

    for _ in range(len(onnx_nodes)+1):
        current_nodes = iter_node(current_nodes, dico_input_tensor, onnx_nodes, constant_nodes, onnx_inputs, onnx_input_names, onnx_weights)
